  ...
  $.ajax({
    ...
    success: function(response){
      for (var i = 0; i < response.users.length; i++) {
        users.push(response.users[i].username);
        const cardHTML = '<div class="card"><div class="card-header" id="heading' + i + '"><h5 class="mb-0"><button class="btn btn-link" data-toggle="collapse" data-target="#collapse' + i + '" aria-expanded="false" aria-controls="collapse' + i + '">' + response.users[i].username + '\'s favourites</button></h5></div><div id="collapse' + i + '" class="collapse" aria-labelledby="heading' + i + '" data-parent="#accordion"><div class="card-body"><div class="container" id="container' + i + '"></div><button type="button" class="btn btn-primary" data-place="' + i + '" data-username="' + response.users[i].username + '" onclick="loadFavourites(this)" >Load Favourites</button></div></div></div>';
